    ...
    const filterSpellsByQuery = ( spells, spellQuery ) => {
        ...
        const activeFilters = Object.keys(spellQuery).filter(key => spellQuery[key] !== null && spellQuery[key] !== '')
            ...
            return activeFilters.every((filterKey) => {
                ...
                switch(filterDataType) {
                    ...
                    case 'number':
                        return spell[filterKey] === spellQuery[filterKey]
                    default:
                        throw new Error('Unexpected data type for SpellQuery filter option. Must be boolean or string')
                }
            })
        })
    }
